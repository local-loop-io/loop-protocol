...
| F-006 | Material search | Required | Implemented | 100% | `/api/v1/material/search` implemented (v0.4.0); dual protocol + Core-DP contract. |
...
| POST | `/api/v1/material/search` | ✅ | `/api/v1/material/search` | Implemented (v0.4.0); protocol + Core-DP dual contract. |
...
None — all openapi.json §8 paths are implemented in `localloop-backend` v0.4.0 (see `localloop-backend/docs/SPEC-COMPLIANCE.md`). Lab boundaries (signature verification, LoopCoin settlement) remain documented there.
